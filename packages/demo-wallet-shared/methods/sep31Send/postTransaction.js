...
export const postTransaction = async ({ token, sendServer, senderId, receiverId, transactionFormData, assetCode, amount, }) => {
    log.instruction({
    ...
    if (![200, 201].includes(result.status)) {
        throw new Error(`POST \`/transactions\` responded with status ${result.status}`);
    }
