...
export const pollTransactionUntilComplete = async ({ sendServer, transactionId, token, }) => {
    log.instruction({
        title: "Polling `/transactions/:id` endpoint until transaction status reaches end status",
    });
    let currentStatus;
    let resultJson;
        ...
        if (currentStatus !== resultJson.transaction.status) {
            ...
            switch (currentStatus) {
                ...
                case TransactionStatus.PENDING_STELLAR:
                    log.instruction({
                        title: "Transaction has been submitted to Stellar network, but is not yet confirmed",
                    });
                    break;
                case TransactionStatus.PENDING_CUSTOMER_INFO_UPDATE:
                    log.instruction({
                        title: "Certain pieces of information need to be updated by the sending anchor",
                    });
                    break;
...
//# sourceMappingURL=pollTransactionUntilComplete.js.map