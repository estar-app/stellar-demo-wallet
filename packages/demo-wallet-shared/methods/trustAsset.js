...
export const trustAsset = async ({ secretKey, untrustedAsset, networkUrl, networkPassphrase, }) => {
    try {
        log.instruction({
        ...
        log.instruction({
            title: "Loading account to get a sequence number for add trustline transaction",
        });
        ...
        const transaction = new TransactionBuilder(account, {
            ...
            asset: new Asset(untrustedAsset.assetCode, untrustedAsset.assetIssuer),
        }))
            .setTimeout(0)
            .build();
