...
import { pollDepositUntilComplete, programmaticDepositFlow, } from "demo-wallet-shared/build/methods/sep6";
...
export const initiateDepositAction = createAsyncThunk("sep6DepositAsset/initiateDepositAction", async (asset, { rejectWithValue, getState }) => {
    ...
    try {
        ...
        const { authentication_required: isAuthenticationRequired, min_amount: minAmount = 0, max_amount: maxAmount = 0, } = assetInfoData;
        let payload = {
            assetCode,
            assetIssuer,
        ...
        if (isAuthenticationRequired) {
            ...
            log.instruction({
                title: "SEP-6 deposit is enabled, and requires authentication so we should go through SEP-10",
            });
...
export const submitSep6DepositFields = createAsyncThunk("sep6DepositAsset/submitSep6DepositFields", async ({ amount, depositType, customerFields, infoFields }, { rejectWithValue, getState }) => {
    try {
...
export const sep6DepositAction = createAsyncThunk("sep6DepositAsset/sep6DepositAction", async (_, { rejectWithValue, getState }) => {
    try {
        const { secretKey } = accountSelector(getState());
        const networkConfig = getNetworkConfig();
        const { data: sep6Data } = sep6DepositSelector(getState());
        const { assetCode, assetIssuer, depositResponse, transferServerUrl, token, } = sep6Data;
