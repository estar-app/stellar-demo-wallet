...
import { pollWithdrawUntilComplete, programmaticWithdrawFlow, } from "demo-wallet-shared/build/methods/sep6";
import { sep10AuthStart, sep10AuthSign, sep10AuthSend, } from "demo-wallet-shared/build/methods/sep10Auth";
import { collectSep12Fields, putSep12FieldsRequest, } from "demo-wallet-shared/build/methods/sep12";
...
export const initiateWithdrawAction = createAsyncThunk("sep6WithdrawAsset/initiateWithdrawAction", async (asset, { rejectWithValue, getState }) => {
    ...
    try {
        ...
        const { authentication_required: isAuthenticationRequired } = assetInfoData;
        let payload = {
            assetCode,
            assetIssuer,
        ...
        if (isAuthenticationRequired) {
            ...
            log.instruction({
                title: "SEP-6 withdrawal is enabled, and requires authentication so we should go through SEP-10",
            });
...
export const submitSep6WithdrawFields = createAsyncThunk("sep6WithdrawAsset/submitSep6WithdrawFields", async ({ withdrawType, infoFields, customerFields }, { rejectWithValue, getState }) => {
    try {
        const { data, secretKey } = accountSelector(getState());
...
export const sep6WithdrawAction = createAsyncThunk("sep6WithdrawAsset/sep6WithdrawAction", async (amount, { rejectWithValue, getState }) => {
    try {
        const { secretKey } = accountSelector(getState());
        const networkConfig = getNetworkConfig();
        const { data: sep6Data } = sepWithdrawSelector(getState());
        const { assetCode, assetIssuer, transferServerUrl, token, withdrawResponse, } = sep6Data;
        const { currentStatus, transaction } = await pollWithdrawUntilComplete({
            amount,
            secretKey,
            transactionId: (withdrawResponse === null || withdrawResponse === void 0 ? void 0 : withdrawResponse.id) || "",
            token,
            transferServerUrl,
            networkPassphrase: networkConfig.network,
            networkUrl: networkConfig.url,
